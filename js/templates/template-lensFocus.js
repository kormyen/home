...
function TemplateLensFocus()
{
  ...
  this.getContent = function(featuredArray)
  {
    let result = ``;
    for (let i = 0; i < Math.min(featuredArray.length, 3); i++)
    {
      let projectInfo = this.projects.get(featuredArray[i].toUpperCase());
      result += projectInfo.HtmlArticle();
    }
    return result;
  }
}